    ...
    return (
        ...
        allComments.map((data)=>(
            <div key={data.comment_id} className='d-flex align-items-start mb-3'>
                <Link to={`/user/profile?id=${data.comment_user}`} className="nav-link fw-semibold d-flex">
                    <img className="rounded-circle border me-3" src={data.pic} alt="img" style={{width: 40, height: 40}}/>
                </Link>
                <div className=''>
                    <div className='pt-2 m-0'>
                        <Link to={`/user/profile?id=${data.comment_user}`} className="nav-link fw-semibold name-link me-2 d-inline">
                        {data.username}
                        </Link>
                        {data.comment}
                    </div>
